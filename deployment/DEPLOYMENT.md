...
| Backend deploy fails: `Failed to fetch Kudu App Settings. Unauthorized (CODE: 401)` | Azure disables SCM Basic Auth Publishing by default on new Function Apps, which breaks publish-profile deploys. Fix: `az resource update --resource-group rg-nutrition-tracker-dev --name scm --namespace Microsoft.Web --resource-type basicPublishingCredentialsPolicies --parent sites/func-nutrition-tracker-dev --set properties.allow=true` (already automated in `infrastructure.yml`/`deploy-infrastructure.sh` going forward) |
